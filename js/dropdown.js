...
.multiselect-dropdown span.maxselected {width: auto;}
...
function MultiselectDropdown(options) {
	...
	document.querySelectorAll('select[multiple]').forEach((el) => {
		...
		const search = newEl('input', { class:['multiselect-dropdown-search'].concat([config.searchInput?.class ?? 'form-control']), style:{ width:'100%', height: '40px', display:el.attributes['multiselect-search']?.value === 'true' ? 'block' : 'none' }, placeholder:config.txtSearch });
		...
		el.loadOptions = () => {
			...
			if (el.attributes['multiselect-select-all']?.value == 'true') {
				...
				const ic = newEl('input', { id: 'all-tag', type:'checkbox', style: { 'accent-color': 'limegreen' } });
				op.appendChild(ic);
				...
				op.addEventListener('click', () => {
					...
					list.querySelectorAll(':scope > div:not(.multiselect-dropdown-all-selector)')
						.forEach(i => {if (i.style.display !== 'none') {clickedAll = true; i.querySelector('input').checked = ch; i.optEl.selected = ch;}});
			...
			Array.from(el.options).map(o => {
				...
				const ic = newEl('input', { type:'checkbox', style: { 'accent-color': 'limegreen' }, checked:o.selected });
				op.appendChild(ic);
			...
			div.refresh = () => {
				...
				if (sels.length > (el.attributes['multiselect-max-items']?.value ?? 5)) {
					...
					sels.map(x => {
						...
						if ((el.attributes['multiselect-hide-x']?.value !== 'true')) {c.appendChild(newEl('span', { class:'optdel', text:'🗙', title:config.txtRemove, onclick:(ev) => {c.srcOption.listitemEl.dispatchEvent(new Event('click'));div.refresh();ev.stopPropagation();} }));}
